...
local full_data_path = string.format("%s/spotify.json", data_path)
...
--- @alias SpotifyRawData {[string]: {[string]: string[]}}

--- @class SpotifyData
--- @field seen {[string]: {[string]: boolean}}
--- @field _data SpotifyRawData
...
function Data:data(key, name)
    if self.has_error then
        error(
            "Spotify: there was an error reading the data file, cannot read data"
        )
    end
...
function Data:update(key, name, values)
    if self.has_error then
        error(
            "Spotify: there was an error reading the data file, cannot update"
        )
    end
...
function Data:sync()
    ...
    local ok, data = pcall(read_data)
    if not ok then
        error("Spotify: unable to sync data, error reading data file")
    end
